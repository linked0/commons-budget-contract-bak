...
export async function setEnvironment(_provider: MockProvider, _commonsBudget: CommonsBudget, _voteraVote: VoteraVote, _admin: Wallet, _voteManager: Wallet, _validators: Wallet[]) {
    provider = _provider;
...
export async function countVote (proposalID: string, positive: number, negative: number, blank: number
): Promise<number[]> {
...
export async function processVote (proposalID: string, positive: number, negative: number, blank: number
): Promise<number[]> {
